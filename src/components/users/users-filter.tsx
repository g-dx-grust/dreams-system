...
/*
 * ユーザー一覧の常設フィルタバー。選択は即時反映（URLクエリを更新）し、
...
export function UsersFilter() {
  ...
  if (sp.get("role"))
    addChip("role", `ロール: ${RoleLabels[sp.get("role") as string] ?? sp.get("role")}`);
  if (sp.get("active"))
    addChip("active", `状態: ${ActiveLabels[sp.get("active") as string] ?? sp.get("active")}`);
    ...
    <div className="flex flex-col gap-s p-m">
      <div className="flex flex-wrap items-end gap-s">
        ...
        <label className="flex flex-col gap-xs">
          <span className="text-s font-medium text-text-grey">ロール</span>
          <Select
            value={sp.get("role") ?? ""}
        ...
        <label className="flex flex-col gap-xs">
          <span className="text-s font-medium text-text-grey">状態</span>
          <Select
            value={sp.get("active") ?? ""}
