...
Excel テンプレートはセル選択作業が中心になるため、Word と同じ 2 カラム編集ではなく
Excel 専用ワークスペースを使う。

- 上部: テンプレート名、未設定数、確認数、完了率
- 中央左: シートタブ付きセルプレビュー
- 中央右: 選択セルのインスペクタ、AI 候補、フィールド辞書
- 下部: マッピング一覧テーブル
- 最下部: 未保存状態と保存ボタンを固定表示
...
納品時の帳票生成画面では、案件に登録された対象土地（`case_parcels.city`）に関連する帳票だけを候補に出します。

- `templates.municipality_id` が未設定のテンプレートは汎用様式として表示する
- `templates.municipality_id` が設定済みのテンプレートは、対象土地の市区町村と一致する場合のみ表示する
- 対象土地が未登録、または市区町村が未入力の場合は、汎用様式のみ表示する
- 一括生成も同じ絞り込みを使い、画面に出ていない自治体専用テンプレートを自動生成しない
