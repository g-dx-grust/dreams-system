...
Lark ログインは **管理者が事前に招待・登録した有効ユーザーのみ許可**する。`/auth/lark/callback` は Lark の user_info から取得したメールアドレスが `public.users.email` に存在し、かつ `is_active = true` の場合だけ Supabase セッションを発行する。未登録・無効アカウントはログイン失敗として `audit_logs` に記録し、Supabase Auth ユーザーを自動作成しない。

メール + パスワードログインも Supabase Auth の認証成功後に `public.users.is_active` を確認し、無効アカウントは即時サインアウトしてログイン失敗として記録する。
...
Supabase Dashboard 側では公開サインアップを無効化し、ユーザー追加は管理者の招待フローまたは初期管理者作成手順に限定する。
...
- Larkテナント同期（テナント全メンバーの一括登録・更新。下記参照）
- 新規ユーザー招待（Supabase Admin API 経由でメール招待。Larkアカウントを持たない外部ユーザー向けの補助手段）
- ロール変更（`user` / `admin`）
- アカウント有効化 / 無効化（`is_active` トグル）

> **パスワードリセット**：Supabase Auth 標準機能（「パスワードを忘れた」リンクから自動送信）を使う。管理者が直接パスワードを設定する機能は実装しない。

### Larkテナント同期（メンバー追加の標準手段）

**目的**：招待メール運用をなくし、Larkテナントに所属するメンバーがそのままLarkログインで利用開始できるようにする。

**仕様**（実装: `src/server/lark-sync.ts` / `src/lib/lark/tenant.ts`）：

- `/users` の「Larkから同期する」ボタン（admin専用）で実行する
- `tenant_access_token` で `contact/v3` API を呼び、ルート部署配下を再帰的に辿って全メンバーを取得する（`open_id` で重複排除）
- 取得した各メンバーを以下のルールで `public.users` へ反映する：
  - `lark_open_id` 一致 → 既存ユーザーとして氏名・アバター・Lark IDを更新（`role` / `is_active` は維持）
  - `email` 一致（`lark_open_id` 未連携のみ）→ 同上＋Lark IDを紐付け
  - 未登録 → `auth.users` を作成（`email_confirm: true`、メール送信なし）し、`role='user'` で登録。メールアドレスを持たないLarkアカウントは `lark-{open_id}@lark.local` を代替メールとする
  - Lark側で退職・凍結済み → `is_active=false` に無効化（新規作成はしない）。自動での再有効化は行わない
- 実行結果（新規・更新・無効化・失敗件数）を `audit_logs` に `user.lark_sync` として記録する

**Larkログインの照合順序**：`users.lark_open_id` → `users.email`。同期済みユーザーはメール変更に影響されずログインできる。

**必要なLarkアプリ権限（スコープ）**：`contact:contact.base:readonly`（テナントメンバー取得）。未付与の場合は同期実行時に日本語エラーで案内する。

## 機微情報ダウンロード

帳票・一括 ZIP・別紙 Excel など、案件情報や関係者情報を含むファイルダウンロード API は以下を必須とする。

- Route Handler 冒頭で `getCurrentUser()` または `requireUser()` を呼び、未認証時は 401 を返す
- `Cache-Control: private, no-store, max-age=0, must-revalidate` と `X-Content-Type-Options: nosniff` を返す
- `Content-Disposition` のファイル名は制御文字・パス区切り文字を除去してから返す
- ZIP 内のエントリ名も同じサニタイズを通し、`../` や `/` による展開先逸脱を防ぐ
- ダウンロード成功時は `audit_logs` に `document.download` としてユーザー・対象 ID・ダウンロード種別・IP アドレスを記録する
...
| `user.lark_sync`                | `syncUsersFromLark` Server Action                  | total, created, updated, deactivated, failed |
